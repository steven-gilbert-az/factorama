                         ...
                         std::vector<Eigen::Vector3d>& landmark_positions)
{
    ...
    std::cout << "Created scenario with " << num_cameras << " cameras and " << num_landmarks << " landmarks\n";
}
...
void RunSolverBenchmark(bool use_2d_bearing_factors)
{
    ...
    OptimizerMethod method_types[] = {OptimizerMethod::GaussNewton, OptimizerMethod::LevenbergMarquardt};
    ...
    for (int method_idx = 0; method_idx < 1; ++method_idx) { // change to 2 to test both gauss and lm
        ...
        if (use_2d_bearing_factors) { // Use BearingProjectionFactor2D for LevenbergMarquardt
            ...
            *graph = CreateGraphWithBearingProjection2D(gt_camera_poses, gt_landmark_positions,
                                                        true,  // random_noise
            ...
            *graph = CreateGraphWithLandmarks(gt_camera_poses, gt_landmark_positions,
                                              true,  // random_noise
                                              false, // constant_pose (let poses optimize)
                                              true,  // prior_factors
                                              0.08,  // noise_sigma (moderate noise)
        ...
        std::cout << "Jacobian size: " << J.rows() << " x " << J.cols() << " (density: " << std::fixed
                  << std::setprecision(2) << 100.0 * J.nonZeros() / double(J.rows() * J.cols()) << "%)\n";
        ...
        std::cout << "  Initial residual: " << std::scientific << std::setprecision(3) << result.initial_residual
                  << "\n";
        std::cout << "  Final residual: " << std::scientific << std::setprecision(3) << result.final_residual << "\n";
        std::cout << "  Total time: " << std::fixed << std::setprecision(2) << result.total_time_ms << " ms\n";
        std::cout << "  Avg time/iter: " << std::fixed << std::setprecision(2) << result.avg_time_per_iter_ms
                  << " ms\n";
    ...
    std::cout << std::left << std::setw(15) << "Method" << std::setw(8) << "Status" << std::setw(6) << "Iters"
              << std::setw(12) << "Final Res" << std::setw(12) << "Total(ms)" << std::setw(12) << "Avg/Iter(ms)"
              << "\n";
    std::cout << std::string(60, '-') << "\n";

    for (const auto& result : results) {
        std::cout << std::left << std::setw(15) << result.name << std::setw(8) << (result.converged ? "OK" : "FAIL")
                  << std::setw(6) << result.iterations << std::setw(12) << std::scientific << std::setprecision(2)
                  << result.final_residual << std::setw(12) << std::fixed << std::setprecision(1)
                  << result.total_time_ms << std::setw(12) << std::fixed << std::setprecision(2)
                  << result.avg_time_per_iter_ms << "\n";
    }

    // Find fastest solver
    auto fastest =
        std::min_element(results.begin(), results.end(), [](const BenchmarkResult& a, const BenchmarkResult& b) {
    ...
    if (fastest != results.end() && fastest->converged) {
        std::cout << "\n🏆 Fastest converged method: " << fastest->name << " (" << std::fixed << std::setprecision(1)
                  << fastest->total_time_ms << " ms)\n";
    }
