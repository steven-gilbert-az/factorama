...
namespace factorama
{
    ...
    PlanePriorFactor::PlanePriorFactor(int id, PlaneVariable *plane, const Eigen::Vector3d& normal_prior,
                                       double distance_prior, double normal_sigma, double distance_sigma)
        : plane_(plane)
    ...
    void PlanePriorFactor::compute_jacobians(std::vector<Eigen::MatrixXd>& jacobians) const
    {
        ...
        if (plane_->is_constant()) {
            ...
            jacobians[0].block<3, 3>(0, 0) =
                weight_normal_ * (n_dot_prior * Eigen::Matrix3d::Identity() + n_current * normal_prior_.transpose() -
